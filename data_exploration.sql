...
Covid 19 Data Exploration 
	...
	Skills used: Joins, CTE's, Variables, Windows Functions, Aggregate Functions, Creating Views

	Using data from Our World In Data, I created a database with 3 tables:
...
FROM
    cases_deaths cd
        LEFT JOIN
    vaccinations vax ON vax.location = cd.location
        AND vax.record_date = cd.record_date
        LEFT JOIN
    demographics demo ON demo.location = cd.location
        AND demo.record_date = cd.record_date
WHERE
...
/* 
Vacination Rates
- The percentage of the population who has received at least one does of the covid vaccine
*/
